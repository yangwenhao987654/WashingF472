...
namespace AutoTF.ZebraUtil
{
    ...
    public static class ZebraPrintHelper
    {
        ...
        public static bool ClearPrintJob(string printName = "ZDesigner ZE500-4 LH-300dpi ZPL")
        {
            TcpIpAddress = "192.168.0.154";
            TcpPort = 9100;
            bool flag = false;
            ...
            flag = SendTCPCmd("192.168.0.154", 9100, cmd);

            // flag = ZebraPrintHelper.PrintCommand(cmd.ToString());
            return flag;
        }

        public static bool ClearPrintJob()
        {
            TcpIpAddress = "192.168.0.154";
            TcpPort = 9100;
            bool flag = false;
        ...
        /// </summary>
        /// <returns></returns>
        public static bool ClearJobs()
        {
            bool flag = false;
            try
            {
                //Open connection
                string printName = "ZDesigner ZE511 RH-300dpi ZPL";
                string cmdStr = "^XA~JA^XZ";
                flag = SendStringToPrinter(printName, cmdStr);
            }
            catch (Exception ex)
            {
                return false;
        ...
        private static bool SendTCPCmd(string IP, int Port, byte[] cmdBytes)
        {
            bool result = false;
            TcpClient tcp = null;
            try
        ...
        private static bool tcpPrint(byte[] cmdBytes)
        {
            ...
            try
            {
                ...
                tcp = new TcpClient("192.168.0.154", 9100);
                tcp.SendTimeout = 1000;
                tcp.ReceiveTimeout = 1000;
                if (tcp.Connected)
                {
